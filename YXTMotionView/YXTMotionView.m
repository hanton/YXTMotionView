...
@interface YXTMotionView () <CRZoomScrollViewDelegate, UIScrollViewDelegate>
...
@property (nonatomic, strong) YXTZoomScrollView *zoomScrollView;
...
- (void)handleTap:(UITapGestureRecognizer *)gesture
{
    ...
    if ([self.contentView isKindOfClass:[UIImageView class]] && self.isZoomEnabled) {
        ...
        self.zoomScrollView = [[YXTZoomScrollView alloc] initFromScrollView:self.scrollView withImage:imageView.image];
...
- (void)setContentView:(UIView *)contentView
{
    ...
    [_scrollView yxt_enableScrollIndicator];
...
- (void)setScrollIndicatorEnabled:(BOOL)scrollIndicatorEnabled
{
    ...
    if (scrollIndicatorEnabled) {
        ...
        [_scrollView yxt_disableScrollIndicator];
    }
}
                                    ...
                                    withHandler:^(CMGyroData *gyroData, NSError *error) {
                                        ...
                                        if (fabs(rotationRate) >= CRMotionViewRotationMinimumTreshold) {
                                            ...
                                            if (!self.stopTracking) {
                                                [UIView animateWithDuration:1.2f//0.3f
                                                                      delay:0.2f//0.0f
...
- (void)dealloc
{
    [self.scrollView yxt_disableScrollIndicator];
}
